...
현재 시스템에는 다음과 같은 고도화된 최적화 기술이 적용되어 있습니다.

#### 1. 100% 규칙 기반 검색 엔진 (Pure Rule-Based)

- **내용**: 기존의 느린 LLM Agent 방식을 전면 폐기하고, 모든 검색 질의를 **Python Regex**로 분석하여 **Neo4j Cypher**로 직통 처리합니다.
- **성과**: 검색 지연 시간(Search Latency) ~150ms 달성 (기존 LLM 방식 대비 99% 단축).

#### 2. Extreme Input Diet (컨텍스트 다이어트)

- **내용**: LLM에게 전달하는 매물 정보(`postgres_details`)에서 '방 설명', '이미지 URL', '임베딩 벡터' 등 무거운 데이터를 모두 제거하고, 핵심 정보(주소, 가격, 옵션)만 남깁니다.
- **기술**: Python 레벨에서 가격(`3000/50`)과 면적(`33m²`)을 미리 포맷팅(Pre-formatting)하여 LLM의 연산 부하를 최소화했습니다.
- **성과**: Input Token 약 60% 감소.

#### 3. Parallel Batch Generation (병렬 분산 생성)

- **내용**: 여러 개의 매물 답변을 순차적(Sequential)으로 생성하던 방식을 **병렬(Parallel) 처리**로 변경했습니다.
- **기술**: LangChain의 `.batch()` 메서드와 ThreadPool을 활용하여, 각 매물에 대한 답변 생성을 동시(Concurrent)에 수행합니다. ("분신술 전략")
- **성과**: 답변 생성 속도 약 3배(300%) 향상. (3개 매물 생성 시 9초 → 3초).
