...
- **학습 데이터**: 2024년 8월 ~ 2025년 8월 월세 실거래가 (1년치)
   - 이유: 최근 1년 데이터를 활용하여 정확성 및 일관성 있음
...
| 구_권역 | 동북/서북/서남/도시/동남권으로 구분 | 거시적 지역 패턴, 서울특별시 도시공간 구간 선정 기준 |
...
| Feature | 설명 | 사용 이유 |
|---------|------|-----------|
| 건물용도 | 아파트/연립다세대/오피스텔 | 건물 유형별 가격 패턴 차이 |
| 임대면적 | 전용면적 (㎡) | 면적과 가격의 직접적 관계 |
| 면적_qcat | 면적 5분위수 구간 (초소형/소형/중소형/중형/대형) | 국토교통부 주택통계에서 활용되는 면적 구간을 기준, 이상치 영향 감소 |
| 층 | 실제 층수 | 층별 가격 차이 (저층 습기, 고층 뷰) |
| 건축연차 | 계약연도 - 건축년도 | 노후도의 연속적 정보 |
| 건축시대 | 건축년도 구간 (2000년 이전/00년대/10년대/20년대 이후) | 주거 수요 구조가 실질적으로 변화하는 10년 단위 건축시대와 결합 |
...
#### 4.1.6 교호작용 Feature (2개)
| Feature | 설명 | 사용 이유 |
|---------|------|-----------|
| 면적_x_건축연차 | 면적 × 건축연차 | 오래된 대형 주택 vs 신축 소형 주택의 가격 구조 차이 |
| 자치구거래량_x_면적 | 자치구 거래량 × 면적 | 면적의 가치가 ‘어느 자치구에 있느냐’에 따라 다르다 |
...
| 모델           |   Accuracy | Precision (macro) | Recall (macro) | **F1-macro** | F1-weighted |
| ------------ | ---------: | ----------------: | -------------: | -----------: | ----------: |
| **LightGBM** | **0.7346** |        **0.7317** |     **0.7393** |   **0.7317** |  **0.7368** |
| XGBoost      |     0.6880 |            0.6883 |         0.6954 |       0.6854 |      0.6904 |
| LSTM         |     0.6704 |            0.6693 |         0.6693 |       0.6715 |      0.6734 |



![alt text](../apps/reco/models/price_model/ML/assets/모델별성능지표.png)
![alt text](../apps/reco/models/price_model/ML/assets/모델별F1-score비교.png)
---

### 6.4 Train / Test 일반화 성능 비교 (F1-macro)
| 모델           |      Train |       Test |
| ------------ | ---------: | ---------: |
| **LightGBM** | **0.8015** | **0.7317** |
| XGBoost      |     0.7278 |     0.6854 |
| LSTM         |     0.7365 |     0.6754 |
...
| --------- | ---------: | --------: | ---------: |
| **저렴(0)** | **11,934** |     1,963 |        349 |
| **적정(1)** |      4,725 | **9,262** |      2,187 |
| **비쌈(2)** |        930 |     6,144 | **14,742** |
...
| 실제 \ 예측   |      저렴(0) |      적정(1) |      비쌈(2) |
| --------- | ---------: | ---------: | ---------: |
| **저렴(0)** | **12,148** |      1,815 |        283 |
| **적정(1)** |      3,948 | **10,200** |      2,026 |
| **비쌈(2)** |        544 |      5,247 | **16,025** |
...
| 실제 \ 예측 | 저렴(0) | 적정(1) | 비쌈(2) |
|---|---:|---:|---:|
| **저렴(0)** | **11826** | 4333 | 1543 |
| **적정(1)** | 2635 | **10268** | 2668 |
| **비쌈(2)** | 936 | 3488 | **10147** |

![alt text](../apps/reco/models/price_model/ML/assets/ML_DL_혼동행렬.png)
...
- 일반화 성능: Train/Test 차이가 적당하고 성능 수준도 가장 높음
- 클래스별 예측 품질: Confusion Matrix를 보면 저렴(0), 적정(1), 비쌈(2) 세 클래스 모두에서 정분류 수가 많음

---

## 7. Shap
![alt text](../apps/reco/models/price_model/ML/shap_plots/bar_test.png)
