"""
중개사 신뢰도 모델 - 다중분류 전체 파이프라인 실행
순서:
1) 데이터 로드 (_00)
2) 타겟 생성 (_01) - 3등급
3) 피처 생성 (_02)
4) 모델 학습 (_03) - 5-Model Ensemble
5) 예측 (_04)
"""
...
# 프로젝트 루트로 작업 디렉토리 변경
# sumi → pipeline → trust_model → models → reco → apps → SKN18-FINAL-1TEAM (7 levels)
project_root = Path(__file__).parent.parent.parent.parent.parent.parent.parent
os.chdir(project_root)
print(f"📁 작업 디렉토리: {project_root}")

# sumi 폴더 경로 추가
sumi_dir = Path(__file__).parent
