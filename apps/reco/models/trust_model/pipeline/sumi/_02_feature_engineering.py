...
def create_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ------------------------------------------------------------
    # 5. 추가 파생 피처 (NEW - 분류 성능 향상용)
    # ------------------------------------------------------------
    # ① 직원당_거래수: 1인당 거래 완료 건수 (생산성 지표)
    df["직원당_거래수"] = df["거래완료"] / df["총_직원수"]
    df["직원당_거래수"] = df["직원당_거래수"].fillna(0).replace([np.inf, -np.inf], 0)
    
    # ② 보조원_자격자_비율: 보조원/자격자 균형 (업무 분담 구조)
    df["보조원_자격자_비율"] = df["중개보조원수"] / df["공인중개사수"].replace(0, 1)
    df["보조원_자격자_비율"] = df["보조원_자격자_비율"].fillna(0).replace([np.inf, -np.inf], 0)
    
    # ③ 경력_규모_상호작용: 규모+경력 시너지 (복합 패턴)
    df["경력규모"] = df["운영연수"] * df["총_직원수"]
    
    # ④ 비자격자_비율: 비자격 인력 비중 (전문성 역지표)
    df["비자격자_비율"] = (df["총_직원수"] - df["공인중개사수"]) / df["총_직원수"]
    df["비자격자_비율"] = df["비자격자_비율"].fillna(0).clip(lower=0)
    
    # ⑤ 연차_카테고리: 운영연수 구간화 (비선형 패턴 캡처)
    ...
    df["연차구간"] = df["운영연수"].apply(get_tenure_category)
