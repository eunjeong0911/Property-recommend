...
def add_features(df):
    ...
    # 영업일수 음수 방지 (혹시 미래 날짜가 들어가 있어도 0으로 처리)
    df["영업일수"] = df["영업일수"].clip(lower=0)

    # 영업년수: 영업일수 / 365.25
    df["영업년수"] = (df["영업일수"] / 365.25).round(2)
    ...
    # 사무소 정보 피처(1개)
    office_staff_count = df.groupby("등록번호")["자격증번호"].nunique()
    df["사무소_인원수"] = df["등록번호"].map(office_staff_count).fillna(0).astype(int)
    
    print("   ✅ 사무소 정보 피처 1개 (사무소_인원수)")
    ...
    # 1인당 등록매물 수
    df["1인당_등록매물수"] = df["등록매물"] / (df["사무소_인원수"] + 1e-6)

    print("   ✅ 파생 피처 4개 (매물_규모_지수, 지역_경쟁_강도, 영업년수_매물_상호작용, 1인당_등록매물수)")
