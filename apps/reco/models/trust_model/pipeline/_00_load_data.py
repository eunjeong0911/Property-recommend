...
def create_office_size_target(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['거래완료율_점수'] = scaler.fit_transform(df[['거래완료율_로그']]) * 70
    ...
    print(f"\n   ⚙️  타겟 구성 (100점 만점):")
    print(f"      - 거래완료율: 70점 (거래 성과 중심)")
    print(f"      - 인력구성: 30점 (공인중개사↑, 중개보조원↓, 법인+)")
    print(f"      ⚠️  경력, 운영 제거 (타겟-피처 일치)")
...
def prepare_ml_data(df: pd.DataFrame) -> tuple:
    ...
    # 1) 복합 점수
    df['전문성_점수'] = (
        df['공인중개사비율'] * 2.0 +      # 가점: 공인중개사 많을수록
        df['법인여부'] * 1.0 -             # 가점: 법인
        df['중개보조원비율'] * 1.5 -       # 감점: 중개보조원 많을수록
        df['중개인비율'] * 0.5             # 약간 가점: 중개인
    )
    
    # 2) 역전 피처 (감점 요인을 명시적으로 표현)
    df['비전문인력비율'] = df['중개보조원비율']  # 높을수록 나쁨
    df['전문인력비율_역전'] = 1 - df['중개보조원비율']  # 낮을수록 좋음
    
    # 3) 대표 비율 (가점)
    df['대표_비율'] = df['대표수'] / df['총_인원수']
    ...
    print(f"   ✅ 사용 피처: 거래성과, 인력구성, 경력, 운영, 지역 (모두 절대값)")
    print(f"   ⚠️  타겟 누수 방지: 비율 피처 제거")
    print(f"      - 거래완료율 ❌ (타겟에 비율 사용)")
    print(f"      - 공인중개사비율 ❌ (타겟에 비율 사용)")
    print(f"      - 법인여부 ❌ (타겟에 여부 사용)")
    print(f"   ✅ 절대값은 OK: 거래완료 건수, 공인중개사 수 등")
    print(f"   💡 타겟: 거래완료율 70% + 인력구성 30%")
