...
def get_feature_list():
    ...
    Target: trust_binary (0/1) - 거래성사율 90% 이상 = 1 (A=고수)
    """
    return [
        # 매물 정보
        "등록매물",
...
def create_base_models():
    ...
    하이브리드 세팅:
      - RF/ET: class_weight='balanced' 로 B(신입) 쪽도 반영
      - GB: 깊이 6 (옛날 강한 버전) + subsample 0.8 (새 버전) 혼합
      - XGB: 규제/샘플링 들어간 새 버전 유지
    """
    models = [
        (
            "rf",
            RandomForestClassifier(
                n_estimators=300,          # 200 -> 300 (안정성)
                max_depth=10,             # 12 -> 10 (약간 규제)
                min_samples_split=6,
                min_samples_leaf=3,
                max_features="sqrt",
                random_state=42,
                n_jobs=-1,
                class_weight="balanced",  # B(신입) 가중치 반영
            ),
        ),
        (
            "gb",
            GradientBoostingClassifier(
                n_estimators=200,
                max_depth=6,              # 🔙 깊이 6 (성능 좋았던 버전)
                learning_rate=0.05,
                subsample=0.8,            # 🔧 새 버전의 subsample 도입 (과적합 완화)
                min_samples_split=6,
                min_samples_leaf=3,
                max_features="sqrt",
                random_state=42,
...
def evaluate_individual_models(models, X_train, X_test, y_train, y_test):
    ...
    for name, model in models:
        ...
        print(
            f"   {display_name:20s} | Test Acc: {test_acc:.3f} | "
            f"Train Acc: {train_acc:.3f} | Gap: {gap:.3f}"
        )
...
def train_binary_ensemble(df):
    ...
    목표: 거래성사율 90% 이상 (A=고수, 1) vs 나머지 (B=신입, 0)
    ...
    if target not in df.columns:
        raise ValueError(
            f"❌ 타겟 컬럼 '{target}' 이(가) 데이터프레임에 없습니다."
        )
    ...
    model_descriptions = {
        "rf": "RandomForest (트리 300개, 깊이 10, class_weight=balanced) ⭐",
        "gb": "GradientBoosting (트리 200개, 깊이 6, LR=0.05, subsample=0.8)",
        "et": "ExtraTrees (트리 300개, 깊이 20, class_weight=balanced)",
        "xgb": "XGBoost (트리 250개, 깊이 5, LR=0.05, subsample=0.8) ⭐",
    }
    ...
    # 가중치 설정 (직전 성능 좋았던 패턴으로 회귀: RF=2, GB=1, ET=1, XGB=2)
    if XGBOOST_AVAILABLE:
    ...
    for name, model in base_models:
        cv_scores = cross_val_score(
            model, X_train_s, y_train, cv=5, scoring="accuracy", n_jobs=-1
        )
        display_name = model_names[name]
        print(
            f"   {display_name:20s} | Acc CV: {cv_scores.mean():.3f} "
            f"(±{cv_scores.std():.3f})"
        )
    ...
    evaluate_individual_models(
        base_models, X_train_s, X_test_s, y_train, y_test
    )
    ...
    print(
        f"   {'Voting Ensemble':20s} | Test Acc: {test_acc:.3f} | "
        f"Train Acc: {train_acc:.3f} | Gap: {gap:.3f}"
    )
    ...
    df["predicted_trust_target"] = np.where(
        y_pred == 1, "A", "B"
    )  # A/B
...
if __name__ == "__main__":
    ...
    print(
        "\n============================================================"
        "\n🚀 중개사 신뢰도 모델 - Voting 앙상블 (이진 분류)"
        "\n   (Soft Voting: 강한 모델만 결합)"
        "\n   (Target: trust_binary 0/1)"
        "\n   (기준: 거래성사율 90% 이상 = A(고수))"
        "\n============================================================"
    )
    ...
    df = load_data()
    df = create_binary_target(df)   # trust_binary 생성 (거래성사율 90% 이상 = 1)
    df = add_features(df)
