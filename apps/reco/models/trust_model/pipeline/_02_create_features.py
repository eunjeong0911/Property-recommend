...
def create_features(df: pd.DataFrame):
    ...
    df["거래완료_safe"] = np.log1p(df["거래완료_숫자"])  # log(1+x) 변환
    df["등록매물_log"] = np.log1p(df["등록매물_숫자"])  # log(1+x) 변환
    df["총거래활동량_log"] = np.log1p(df["거래완료_숫자"] + df["등록매물_숫자"])  # log(1+x) 변환
    
    # 4-6. 인력 지표
    df["총_직원수"] = df["총_직원수"]
    df["총_직원수_safe"] = df["총_직원수"].replace(0, 1)  # 0으로 나누기 방지
    df["공인중개사수"] = df["공인중개사수"]
    ...
    # ------------------------------------------------------------
    # 13. 중개보조원_비율: 보조 인력 비중
    df["중개보조원_비율"] = df["중개보조원수"] / df["총_직원수_safe"]
    
    # 14. 베테랑: 5년 이상 운영 (이진)
    df["베테랑"] = (df["운영기간_년"] >= 5).astype(int)
    
    # 15. 경력_규모_지수: 경력과 규모 시너지
    df["경력_규모_지수"] = df["운영기간_년"] * df["총_직원수"]
