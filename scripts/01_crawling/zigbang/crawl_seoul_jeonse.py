...
사용법:
    python crawl_seoul_jeonse.py          # 서울 전역
    python crawl_seoul_jeonse.py --test   # 테스트 (100개만)
"""
...
SEOUL_GEOHASHES = [
    ...
    "wydm2", "wydmb", "wydms", "wydq5", "wydq7",
    "wydjp", "wydjq", "wydjr", "wydjw", "wydmt",
    "wydmk", "wydq4", "wydq1",
]
...
def get_item_ids_by_geohash(geohash: str) -> List[int]:
    ...
    for retry in range(MAX_RETRIES):
        try:
            ...
            print(f"  [geohash: {geohash}] {len(item_ids)}개 전세 매물 ID 발견")
            return item_ids
    ...
    if os.path.exists(output_file):
        try:
            with open(output_file, "r", encoding="utf-8") as f:
                data = json.load(f)
                return {item["매물번호"] for item in data}
        except:
            pass
    return set()
...
def crawl_seoul_jeonse(max_items: int = None):
    """서울 전역 전세 원룸 크롤링 (병렬 처리)"""
    print("=" * 60)
    print("🏠 직방 서울 전세 원룸 크롤링 시작")
    ...
    print(f"\n✅ 총 {len(all_item_ids)}개 고유 전세 매물 ID 수집 완료\n")
    ...
    output_file = os.path.join(OUTPUT_DIR, "zigbang_seoul_jeonse.json")
    ...
    if transformed_items:
        ...
        print(f"   - 전세금: {sample['거래_정보']['전세금']}만원")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--test":
        crawl_seoul_jeonse(max_items=100)
    else:
        crawl_seoul_jeonse()
